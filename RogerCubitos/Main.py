...
def produceTitle():
    our_font = pygame.font.SysFont("monospace", 35)
    title="Juego de DAdotes !!"
    TITLE = our_font.render(title, 1, white)
    gameDisplay.blit(TITLE, (SCREEN_WIDTH/8, SCREEN_HEIGHT/5))

# our roll will display message with our roll converted to text form, alongside
def before_roll():
    produce_button_message("Ingresen sus nombres")

def roll():
    #Completed roll Message. Cast int to str to output the message clearly
    text = " P1 ->> You've completed your roll " + str(FIRST_DICE) + ","  + str(SECOND_DICE)+ ","  + str(TRD_DICE) + ","  + str(F4RT_DICE)+ ","  + str(F5FT_DICE)
    print(text)
    produce_roll_message(text)

def roll_p2():
    text = " P2 ->> You've completed your roll " + str(P2_FIRST_DICE) + ","  + str(P2_SECOND_DICE)+ ","  + str(P2_TRD_DICE) + ","  + str(P2_F4RT_DICE)+ ","  + str(P2_F5FT_DICE)
    print(text)
...
while already_rolled == False: 
    ...
    p2display_dice(P2_FIRST_DICE,P2_SECOND_DICE,P2_TRD_DICE,P2_F4RT_DICE,P2_F5FT_DICE)
